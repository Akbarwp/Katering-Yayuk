            ...
            <div class="w-full pt-16 px-6">
                <div class="mx-auto text-center">
                    <h1 class="mb-5 text-dark text-5xl font-bold">
                        Daftar Paket
                        <a href="paket/tambahh" class="btn btn-circle btn-sm btn-success ml-2 hover:bg-opacity-80">
                        ...
                        <table class="table table-zebra w-2/3 mx-auto text-center justify-center text-light">
                            ...
                            <tbody>
                                ...
                                @foreach ($paket as $pkt)
                                    <tr>
                                        <th class="bg-primary">{{ $pkt->id }}</th>
                                        <td class="bg-primary">
                                            <ul>{{ $pkt->nasi }}</ul>
                                            <ul>{{ $pkt->lauk_utama }}</ul>
                                            <ul>{{ $pkt->lauk_pendamping1 }}</ul>
                                            <ul>{{ $pkt->lauk_pendamping2 }}</ul>
                                        </td>
                                        <td class="bg-primary">{{ $pkt->harga }}</td>
                                        <td class="bg-primary">
                                            <a href="paket/ubahh/{{ $pkt->id }}"
                                                class="btn btn-warning mr-2 hover:bg-opacity-80">Ubah</a>
                                            <a href="paket/hapuss/{{ $pkt->id }}"
