...
@section('container')
    <section id="homeAlternatif" class="bg-light pb-36">
        <div class="container font-sarabun">
            <div class="w-full pt-16 px-6">
                <div class="mx-auto text-center">
                    <h1 class="mb-5 text-dark text-5xl font-bold">
                        Daftar Alternatif
                        <a href="alternatif/tambahh" class="btn btn-circle btn-sm btn-success ml-2 hover:bg-opacity-80">
                        ...
                        <table class="table table-zebra w-2/3 mx-auto text-center justify-center text-light">
                            ...
                            <thead class="text-light">
                                <tr>
                                    ...
                                    <th class="text-base bg-dark">ID Paket</th>
                                    @foreach ($kriteria as $kri)
                                        <th class="text-base bg-dark">Sub Kriteria {{ $kri->nama }}</th>
                                    @endforeach
                            ...
                            <tbody>
                                ...
                                @foreach ($alternatif as $alt)
                                    <tr>
                                        <th class="bg-primary">{{ $alt->id }}</th>
                                        <td class="bg-primary">{{ $alt->id_paket }}</td>
                                        @foreach ($namaKriteria as $nk)
                                            <td class="bg-primary">{{ $alt->$nk }}</td>
                                        @endforeach
                                        <td class="bg-primary">
                                            <a href="alternatif/ubahh/{{ $alt->id }}"
                                                class="btn btn-warning mr-2 hover:bg-opacity-80">Ubah</a>
                                            <a href="alternatif/hapuss/{{ $alt->id }}"
